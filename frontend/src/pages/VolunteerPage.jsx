...
const DISTRICTS_BR = [
  ...
  "Ara","Begusarai","Nalanda","Chapra","Munger","Other",
];

const LANGUAGES = ["Hindi","Bhojpuri","Maithili","Magahi","Urdu","English","Other"];
    ...
    <div className="vf-step-body">
      <Field label="District" required error={errors.district}>
        <select className={`vf-input ${errors.district ? "vf-input--err" : ""}`}
          ...
          {DISTRICTS_BR.map(d => <option key={d}>{d}</option>)}
        </select>
      </Field>
      ...
      <Field label="PIN Code">
        <input className="vf-input" value={form.pincode}
          ...
          placeholder="e.g. 800001" maxLength={6} />
      </Field>
...
export default function VolunteerPage() {
  ...
  const handleSubmit = async () => {
    ...
    // Field names match what the backend /api/volunteers route validates.
    const payload = {
      firstName:    form.firstName,
      lastName:     form.lastName,
      email:        form.email,
      phone:        form.phone,           // 10 digits; backend assumes +91
      dob:          form.dob,
      skills:       form.skills,          // arrays sent as-is
    ...
    try {
      const res = await api.submitVolunteer(payload);
      // If a CV was attached, upload it to the new application.
      if (form.cvFile && res?.id) {
        try {
          await api.uploadVolunteerCV(res.id, form.cvFile);
        } catch (upErr) {
          // The application is saved; a CV failure shouldn't block success.
